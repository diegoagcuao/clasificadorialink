            ...
            if (response.length === 0) {
                ...
                response.forEach(function(enlace) {
                    $('#listaEnlaces' + listaId).append(
                        ...
                        '<a href="' + enlace.href + '" target="_blank">' + enlace.titulo + ' (' + enlace.categoria + ')</a>' +
                        '<button class="btn btn-danger btn-sm ml-auto" onclick="eliminarEnlaceDeLista(' + enlace.id + ', ' + listaId + ')">Eliminar</button>' +
                        '</li>'
                    );
                });
            }
